	...
	void assertEntities(final E actual, final E expected) {
		...
		if (actual != null) {
			if (actual instanceof Company) {
				assertCompanies((Company) actual, (Company) expected);
			} else if (actual instanceof Department) {
				assertDepartments((Department) actual, (Department) expected);
			}
		}
	}

	protected static void assertCompanies(final Company actual, final Company expected) {
		assertEquals(actual.getId(), expected.getId());
		assertEquals(actual.getName(), expected.getName());
		assertEquals(actual.getRegisteredAt(), expected.getRegisteredAt());
	}

	protected static void assertDepartments(final Department actual, final Department expected) {
		assertEquals(actual.getId(), expected.getId());
		assertEquals(actual.getName(), expected.getName());
